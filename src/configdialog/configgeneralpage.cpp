    ...
    , config(_config)
{
    ...
    iNumFiles->setToolTip(
        i18n("You shouldn't set this number higher than the amount of installed processor cores.\nThere have been %1 processor cores detected.",
             processorsCount));
    ...
    iNumReplayGainFiles->setToolTip(
        i18n("You shouldn't set this number higher than the amount of installed processor cores.\nThere have been %1 processor cores detected.",
             processorsCount));
...
{
    const bool changed = cStartTab->currentIndex() != config->data.general.startTab || cDefaultProfile->currentText() != config->data.general.defaultProfile
        || cDefaultFormat->currentText() != config->data.general.defaultFormat
        || cConflictHandling->currentIndex() != (int)config->data.general.conflictHandling || iNumFiles->value() != config->data.general.numFiles
        || cWaitForAlbumGain->isChecked() != config->data.general.waitForAlbumGain || cCopyIfSameCodec->isChecked() != config->data.general.copyIfSameCodec
