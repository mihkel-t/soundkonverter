    ...
    for (int i = 0; i < formatInfos.count(); i++) {
        if (formatInfos.at(i).codecName == codecName) {
            ...
            if (formatInfos.at(i).lossless != info.lossless)
                logger->log(1000,
                            "<span style=\"color:red\">Disturbing Error: Plugin " + plugin->name() + " says " + codecName + " was "
                                + (info.lossless ? "lossless" : "lossy") + " but it is already registed as " + (!info.lossless ? "lossless" : "lossy")
                                + "</span>");
...
void PluginLoader::load()
{
    ...
    const QVector<KPluginMetaData> codecsPlugins = KPluginMetaData::findPlugins("soundkonverterplugins/codecs");
            ...
            if (encodeCodecs.count() > 0) {
                ...
                for (int j = 0; j < encodeCodecs.count(); j++) {
                    ...
                    logger->log(
                        ...
                        "<pre>\t\t\t"
                            + QString("%1%2(%3)")
                                  .arg(encodeCodecs.keys().at(j))
                                  .arg(spaces)
                                  .arg(encodeCodecs.values().at(j) ? "<span style=\"color:green\">enabled</span>" : "<span style=\"color:red\">disabled</span>")
                            + "</pre>");
                }
            }
            if (decodeCodecs.count() > 0) {
                ...
                for (int j = 0; j < decodeCodecs.count(); j++) {
                    ...
                    logger->log(
                        ...
                        "<pre>\t\t\t"
                            + QString("%1%2(%3)")
                                  .arg(decodeCodecs.keys().at(j))
                                  .arg(spaces)
                                  .arg(decodeCodecs.values().at(j) ? "<span style=\"color:green\">enabled</span>" : "<span style=\"color:red\">disabled</span>")
                            + "</pre>");
                }
            }
            logger->log(1000, "");
    ...
    for (const KPluginMetaData &filterPlugin : filterPluginsVect) {
        if (auto pluginResult = KPluginFactory::instantiatePlugin<FilterPlugin>(filterPlugin, this, QVariantList())) {
            ...
            logger->log(1000, "<pre>\t<span style=\"color:red\">failed to load plugin: " + filterPlugin.fileName() + "</span></pre>");
        }
    }
    ...
    foreach (const ConversionPipeTrunk &trunk, conversionFilterPipeTrunks) {
        ...
        if (possibilities & Encode) {
            ...
            if (((compressionType & Lossy && !isLossless) || (compressionType & Lossless && isLossless))
                ...
                set += codec;
            //             if( compressionType & Hybrid && isCodecHybrid(codec) && ( compressionType & InferiorQuality && isInferiorQuality ||
            //             !isInferiorQuality ) )
            //                 set += codec;
        }
...
QList<ConversionPipe>
PluginLoader::getConversionPipes(const QString &codecFrom, const QString &codecTo, QList<FilterOptions *> filterOptions, const QString &preferredPlugin)
{
        ...
        if (conversionFilterPipeTrunks.at(i).codecFrom == codecFrom && conversionFilterPipeTrunks.at(i).enabled) {
            if (codecFrom == "wav" && conversionFilterPipeTrunks.at(i).codecTo == codecTo) {
                ...
                for (int j = 0; j < conversionFilterPipeTrunks.count(); j++) {
                    ...
                    if (conversionFilterPipeTrunks.at(j).codecFrom == conversionFilterPipeTrunks.at(i).codecTo
                        && conversionFilterPipeTrunks.at(j).codecTo == codecTo && conversionFilterPipeTrunks.at(j).enabled) {
                        ConversionPipe newPipe;
...
QString PluginLoader::getCodecFromM4aFile(QFile *file)
{
    ...
    while (!file->atEnd()) {
        ...
        if (atomPathDepth == 6 && name == "mp4a") {
            ...
            qint64 int_length = ((static_cast<qint64>(length.at(0)) & 0xFF) << 24) + ((static_cast<qint64>(length.at(1)) & 0xFF) << 16)
                + ((static_cast<qint64>(length.at(2)) & 0xFF) << 8) + ((static_cast<qint64>(length.at(3)) & 0xFF));
            ...
            if (int_length == 1) // Meaning: length is 64 bits
            {
                ...
                int_length = ((static_cast<qint64>(l.at(0)) & 0xFF) << 56) + ((static_cast<qint64>(l.at(1)) & 0xFF) << 48)
                    + ((static_cast<qint64>(l.at(2)) & 0xFF) << 40) + ((static_cast<qint64>(l.at(3)) & 0xFF) << 32)
                    + ((static_cast<qint64>(l.at(4)) & 0xFF) << 24) + ((static_cast<qint64>(l.at(5)) & 0xFF) << 16)
                    + ((static_cast<qint64>(l.at(6)) & 0xFF) << 8) + ((static_cast<qint64>(l.at(7)) & 0xFF));
            }
...
bool PluginLoader::canReplayGain(const QString &codecName, CodecPlugin *plugin, QStringList *errorList)
{
    ...
    if (plugin) {
        for (int i = 0; i < conversionFilterPipeTrunks.size(); i++) {
            if (conversionFilterPipeTrunks.at(i).plugin == plugin && conversionFilterPipeTrunks.at(i).codecTo == codecName
                && conversionFilterPipeTrunks.at(i).enabled && conversionFilterPipeTrunks.at(i).data.hasInternalReplayGain) {
                return true;
            }
        }
    }
...
bool PluginLoader::canRipEntireCd(QStringList *errorList)
{
    for (int i = 0; i < conversionFilterPipeTrunks.count(); i++) {
        if (conversionFilterPipeTrunks.at(i).plugin->type() == "ripper" && conversionFilterPipeTrunks.at(i).data.canRipEntireCd
            && conversionFilterPipeTrunks.at(i).enabled) {
            return true;
        }
    }
...
QMap<QString, QStringList> PluginLoader::decodeProblems(bool detailed)
{
    ...
    for (int i = 0; i < conversionFilterPipeTrunks.size(); i++) {
        if (!conversionFilterPipeTrunks.at(i).enabled && !conversionFilterPipeTrunks.at(i).problemInfo.isEmpty()
            && !problems.value(conversionFilterPipeTrunks.at(i).codecFrom).contains(conversionFilterPipeTrunks.at(i).problemInfo)
            && !enabledCodecs.contains(conversionFilterPipeTrunks.at(i).codecFrom)) {
...
QMap<QString, QStringList> PluginLoader::encodeProblems(bool detailed)
{
    ...
    for (int i = 0; i < conversionFilterPipeTrunks.size(); i++) {
        if (!conversionFilterPipeTrunks.at(i).enabled && !conversionFilterPipeTrunks.at(i).problemInfo.isEmpty()
            && !problems.value(conversionFilterPipeTrunks.at(i).codecTo).contains(conversionFilterPipeTrunks.at(i).problemInfo)
            && !enabledCodecs.contains(conversionFilterPipeTrunks.at(i).codecTo)) {
...
QMap<QString, QStringList> PluginLoader::replaygainProblems(bool detailed)
{
    ...
    for (int i = 0; i < replaygainPipes.size(); i++) {
        if (!replaygainPipes.at(i).enabled && !replaygainPipes.at(i).problemInfo.isEmpty()
            && !problems.value(replaygainPipes.at(i).codecName).contains(replaygainPipes.at(i).problemInfo)
            && !enabledCodecs.contains(replaygainPipes.at(i).codecName)) {
...
QString PluginLoader::pluginDecodeProblems(const QString &pluginName, const QString &codecName)
{
    for (int i = 0; i < conversionFilterPipeTrunks.size(); i++) {
        if (!conversionFilterPipeTrunks.at(i).enabled && !conversionFilterPipeTrunks.at(i).problemInfo.isEmpty()
            && conversionFilterPipeTrunks.at(i).plugin->name() == pluginName && conversionFilterPipeTrunks.at(i).codecFrom == codecName) {
...
QString PluginLoader::pluginEncodeProblems(const QString &pluginName, const QString &codecName)
{
    for (int i = 0; i < conversionFilterPipeTrunks.size(); i++) {
        if (!conversionFilterPipeTrunks.at(i).enabled && !conversionFilterPipeTrunks.at(i).problemInfo.isEmpty()
            && conversionFilterPipeTrunks.at(i).plugin->name() == pluginName && conversionFilterPipeTrunks.at(i).codecTo == codecName) {
            return conversionFilterPipeTrunks.at(i).problemInfo;
        }
    }
...
{
    for (int i = 0; i < replaygainPipes.size(); i++) {
        if (!replaygainPipes.at(i).enabled && !replaygainPipes.at(i).problemInfo.isEmpty() && replaygainPipes.at(i).plugin->name() == pluginName
            && replaygainPipes.at(i).codecName == codecName) {
...
{
    for (int i = 0; i < conversionPipeTrunks.count(); i++) {
        if (conversionPipeTrunks.at(i).codecTo == codecName && conversionPipeTrunks.at(i).plugin->type() == "codec"
            && conversionPipeTrunks.at(i).data.hasInternalReplayGain) {
            return true;
        }
    }
    return false;
