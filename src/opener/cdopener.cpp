    ...
    , cddbFound(false)
{
    ...
    if (!cEntireCd->isEnabled()) {
        ...
        if (!errorList.isEmpty()) {
            errorList.prepend(
                i18n("Ripping an entire cd to a single file is not supported by the installed backends.\nPossible solutions are listed below.\n"));
        } else {
            errorList += i18n(
                "Ripping an entire cd to a single file is not supported by the installed backends.\nPlease check your distribution's package manager in order "
                "to install an additional ripper plugin which supports ripping to one file.");
        }
...
void CDOpener::lookup_cddb_done(KCDDB::Result result)
{
    ...
    if (result != KCDDB::Success && result != KCDDB::MultipleRecordFound) {
        ...
        // Error(i18n("No entry found in CDDB."), i18n("This means no data found in the CDDB database. Please enter the data manually. Maybe try another CDDB
    ...
    if (cddb->lookupResponse().count() > 1 || cdTextFound) {
        ...
        for (int i = 0; i < cddb_info.count(); i++) {
            list.append(QString("%1, %2, %3")
                            .arg(cddb_info.at(i).get(KCDDB::Artist).toString())
                            .arg(cddb_info.at(i).get(KCDDB::Title).toString())
                            .arg(cddb_info.at(i).get(KCDDB::Genre).toString()));
        }
