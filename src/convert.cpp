...
void Convert::get(ConvertItem *item)
{
    ...
    logger->log(item->logID, i18n("Copying \"%1\" to \"%2\"", item->inputUrl.url(), item->tempInputUrl.toLocalFile()));

    item->kioCopyJob = KIO::file_copy(item->inputUrl, item->tempInputUrl, -1, KIO::HideProgressInfo);
    connect(item->kioCopyJob.data(), &KIO::FileCopyJob::result, this, &Convert::kioJobFinished);
    connect(item->kioCopyJob.data(), &KIO::FileCopyJob::percentChanged, this, &Convert::kioJobProgress);
}
...
void Convert::convert(ConvertItem *item)
{
    ...
    if (item->outputUrl.isEmpty()) {
        // item->outputUrl = !item->fileListItem->outputUrl.url().isEmpty() ? item->fileListItem->outputUrl : OutputDirectory::calcPath( item->fileListItem,
        // config, usedOutputNames.values() );
    ...
    if (item->conversionPipes.at(item->take).trunks.count() == 1) // conversion can be done by one plugin alone
    {
        ...
        if (item->backendPlugin->type() == "codec" || item->backendPlugin->type() == "filter") {
            ...
            item->backendID = qobject_cast<CodecPlugin *>(item->backendPlugin)
                                  ->convert(inputUrl,
                                            item->outputUrl,
                                            item->conversionPipes.at(item->take).trunks.at(0).codecFrom,
                                            item->conversionPipes.at(item->take).trunks.at(0).codecTo,
                                            conversionOptions,
                                            item->fileListItem->tags,
                                            useInternalReplayGain);
        } else if (item->backendPlugin->type() == "ripper") {
            item->backendID = qobject_cast<RipperPlugin *>(item->backendPlugin)
                                  ->rip(item->fileListItem->device, item->fileListItem->track, item->fileListItem->tracks, item->outputUrl);
        }
    ...
    } else // conversion needs two plugins or more
    {
        ...
        if (config->data.advanced.usePipes) {
            ...
            foreach (const ConversionPipeTrunk &trunk, item->conversionPipes.at(item->take).trunks) {
                ...
                if (plugin->type() == "codec" || plugin->type() == "filter") {
                    ...
                    command = qobject_cast<CodecPlugin *>(plugin)->convertCommand(inUrl,
                                                                                  outUrl,
                                                                                  item->conversionPipes.at(item->take).trunks.at(step).codecFrom,
                                                                                  item->conversionPipes.at(item->take).trunks.at(step).codecTo,
                                                                                  conversionOptions,
                                                                                  item->fileListItem->tags,
                                                                                  useInternalReplayGain);
                } else if (plugin->type() == "ripper") {
                    command = qobject_cast<RipperPlugin *>(plugin)->ripCommand(item->fileListItem->device,
                                                                               item->fileListItem->track,
                                                                               item->fileListItem->tracks,
                                                                               outUrl);
                }
                if (command.isEmpty()) {
                    usePipes = false;
        ...
        if (usePipes) {
            ...
            const bool useSharedMemory = config->data.advanced.useSharedMemoryForTempFiles && estimatedFileSize > 0
                && estimatedFileSize < config->data.advanced.maxSizeForSharedMemoryTempFiles;

            for (int i = 0; i < item->conversionPipes.at(item->take).trunks.count() - 1; i++) {
                item->tempConvertUrls +=
                    item->generateTempUrl("convert",
                                          config->pluginLoader()->codecExtensions(item->conversionPipes.at(item->take).trunks.at(i).codecTo).first(),
                                          useSharedMemory);
            }
...
void Convert::convertNextBackend(ConvertItem *item)
{
    ...
    if (plugin->type() == "codec" || plugin->type() == "filter") {
        ...
        item->backendID = qobject_cast<CodecPlugin *>(plugin)->convert(inUrl,
                                                                       outUrl,
                                                                       item->conversionPipes.at(item->take).trunks.at(step).codecFrom,
                                                                       item->conversionPipes.at(item->take).trunks.at(step).codecTo,
                                                                       conversionOptions,
...
void Convert::replaygain(ConvertItem *item)
{
    ...
    foreach (ConvertItem *albumItem, albumItems) {
        ...
        directories.append(albumItem->outputUrl.adjusted(QUrl::RemoveFilename).path());
        if (albumItem != item) {
...
void Convert::writeTags(ConvertItem *item)
{
    ...
    if (config->data.coverArt.writeCovers == 0 || (config->data.coverArt.writeCovers == 1 && !success)) {
        config->tagEngine()->writeCoversToDirectory(item->outputUrl.adjusted(QUrl::RemoveFilename).path(), item->fileListItem->tags);
    }
}
...
void Convert::pluginProcessFinished(int id, int exitCode)
{
    ...
    foreach (ConvertItem *item, items) {
        if (item->backendPlugin && item->backendPlugin == QObject::sender() && item->backendID == id) {
            ...
            if (item->backendPlugin->name() == "Vorbis Gain") {
                activeVorbisGainDirectories.removeAll(item->outputUrl.adjusted(QUrl::RemoveFilename).path());
...
void Convert::add(FileListItem *fileListItem)
{
    ...
    if (fileListItem->track >= 0) {
        if (fileListItem->tags) {
            fileName = QUrl(i18nc("identificator for the logger",
                                  "CD track %1: %2 - %3",
                                  QString().asprintf("%02i", fileListItem->tags->track),
                                  fileListItem->tags->artist,
                                  fileListItem->tags->title));
    ...
    logger->log(1000, i18n("Adding new item to conversion list: '%1'", fileName.toDisplayString(QUrl::PreferLocalFile)));
    ...
    newItem->logID = logger->registerProcess(fileName.toDisplayString(QUrl::PreferLocalFile));
    ...
    newItem->conversionPipes = config->pluginLoader()->getConversionPipes(fileListItem->codecName,
                                                                          conversionOptions->codecName,
                                                                          conversionOptions->filterOptions,
    ...
    for (int i = 0; i < newItem->conversionPipes.size(); i++) {
        ...
        for (int j = 0; j < newItem->conversionPipes.at(i).trunks.size(); j++) {
            pipe_str += QString("%1 %2 %3 (%4)")
                            .arg(newItem->conversionPipes.at(i).trunks.at(j).codecFrom)
                            .arg(QChar(10141))
                            .arg(newItem->conversionPipes.at(i).trunks.at(j).codecTo)
                            .arg(newItem->conversionPipes.at(i).trunks.at(j).plugin->name());
        }
...
void Convert::remove(ConvertItem *item, FileListItem::ReturnCode returnCode)
{
    ...
    const ConversionOptions *conversionOptions = config->conversionOptionsManager()->getConversionOptions(item->fileListItem->conversionOptionsId);
    if (fileRatio < 0.01 && outputFileInfo.size() < 100000 && returnCode != FileListItem::StoppedByUser
        && (!conversionOptions || !config->pluginLoader()->isCodecInferiorQuality(conversionOptions->codecName))) {
    ...
    if (!waitForAlbumGain && !item->fileListItem->notifyCommand.isEmpty()
        && (!config->data.general.waitForAlbumGain || !conversionOptions || !conversionOptions->replaygain)) {
        QList<ConvertItem *> albumItems;
        if (!albumName.isEmpty())
            albumItems = albumGainItems[albumName];
    ...
    if (returnCode != FileListItem::Succeeded && returnCode != FileListItem::SucceededWithProblems && returnCode != FileListItem::Skipped
        && QFile::exists(item->outputUrl.toLocalFile())) {
        logger->log(item->logID, i18n("Removing partially converted output file"));
        QFile::remove(item->outputUrl.toLocalFile());
    }
...
void Convert::kill(FileListItem *fileListItem)
{
    for (int i = 0; i < items.size(); i++) {
        if (items.at(i)->fileListItem == fileListItem) {
            ...
            } else if (items.at(i)->process) {
                items.at(i)->process->kill();
