    ...
    , config(_config)
{
    ...
    //     paste = new QAction( i18n("Paste"), "editpaste", 0, this, 0, actionCollection, "paste" );  // TODO paste
...
FileList::~FileList()
{
    ...
    /*
    if (!QApplication::sessionSaving()) {
        QFile listFile( QStandardPaths::locate(QStandardsPaths::AppLocalDataDir, "filelist_autosave.xml") );
        listFile.remove();
    }*/
}
    ...
    for (int i = 0; i < q_urls.size(); i++) {
        ...
        if (mimeType == "inode/directory" || config->pluginLoader()->canDecode(codecName, &errorList)) {
            ...
            if (mimeType == "application/x-ole-storage" || // Thumbs.db
                mimeType == "application/x-wine-extension-ini" || mimeType == "application/x-cue" || mimeType == "application/x-k3b"
                || mimeType == "application/pdf" || mimeType == "application/x-trash" || mimeType.startsWith("application/vnd.oasis.opendocument")
                || mimeType.startsWith("application/vnd.openxmlformats-officedocument") || mimeType.startsWith("application/vnd.sun.xml"))
                continue;

            fileName = QUrl(q_urls.at(i)).toDisplayString(QUrl::PreferLocalFile);
                        ...
                        int conversionOptionsId)
{
    ...
    foreach (const QUrl &fileName, fileList) {
        ...
        newItem->local = (newItem->url.isLocalFile() || newItem->url.scheme() == "file");
...
void FileList::addTracks(const QString &device,
                         QList<int> trackList,
                         int tracks,
...
void FileList::updateItem(FileListItem *item)
{
    ...
    if (item->track >= 0) {
        if (item->tags) {
            item->setText(Column_Input, QString().asprintf("%02i", item->tags->track) + " - " + item->tags->artist + " - " + item->tags->title);
        } else // shouldn't be possible
        {
            item->setText(Column_Input, i18n("CD track %1", item->track));
        }
    } else {
        item->setText(Column_Input, item->url.toDisplayString(QUrl::PreferLocalFile));
    }
}
...
bool FileList::waitForAlbumGain(
    FileListItem *item) // TODO was ist wenn die datei unten drunter schon fertig ist, was ist wenn eine datei oben drpber noch nicht fertig ist
{
...
void FileList::load(bool user)
{
    ...
    load(QStandardPaths::locate(QStandardPaths::AppLocalDataLocation, fileListPath));
}
...
void FileList::load(const QString &fileListPath)
{
    if (topLevelItemCount() > 0) {
        const int ret = KMessageBox::questionTwoActions(this,
                                                        ...
                                                        i18n("If not, the saved file list will be appended."),
        ...
        if (list.setContent(&listFile)) {
            ...
            if (root.nodeName() == "soundkonverter" && root.attribute("type") == "filelist") {
                ...
                for (int i = 0; i < conversionOptionsElements.count(); i++) {
                    ...
                    if (plugin) {
                        ConversionOptions *conversionOptions =
                            plugin->conversionOptionsFromXml(conversionOptionsElements.at(i).toElement(), &filterOptionsElements);
                        if (conversionOptions) {
...
void FileList::save(bool user)
{
    ...
    for (int i = 0; i < topLevelItemCount(); i++) {
        ...
        file.setAttribute("url", item->url.toDisplayString(QUrl::PreferLocalFile));
        // file.setAttribute("outputUrl",item->outputUrl.toDisplayString(QUrl::PreferLocalFile));
