    ...
    if (event->source() == this) {
        ...
        foreach (const QUrl &url, q_urls) {
            ...
            if (mimeType == "inode/directory") {
                ...
                fileName = QUrl(url).toDisplayString(QUrl::PreferLocalFile);
                ...
                if (!errorList.isEmpty()) {
                    ...
                    problems[codecName][1] += i18n(
                        ...
                        "at your distribution's package manager for this.");
                }
            }
        }
    ...
    foreach (const QUrl &url, fileList) {
        ...
        if (tags && !tags->album.simplified().isEmpty()) {
            ...
            for (int j = 0; j < topLevelItemCount(); j++) {
                if (topLevelItem(j)->type == ReplayGainFileListItem::Album && topLevelItem(j)->codecName == codecName
                    && topLevelItem(j)->samplingRate == samplingRate
                    && ((config->data.general.replayGainGrouping == Config::Data::General::AlbumDirectory && topLevelItem(j)->albumName == tags->album
                         && topLevelItem(j)->url.toLocalFile() == url.adjusted(QUrl::RemoveFilename).path())
                        || (config->data.general.replayGainGrouping == Config::Data::General::Album && topLevelItem(j)->albumName == tags->album)
            ...
            if (!newTrackItem) {
                ...
                if (config->data.general.replayGainGrouping == Config::Data::General::AlbumDirectory) {
                    ...
                    newAlbumItem->albumName = url.adjusted(QUrl::RemoveFilename).path();
                }
...
void ReplayGainFileList::updateItem(ReplayGainFileListItem *item, bool initialUpdate)
{
    ...
    if (item->type == ReplayGainFileListItem::Album) {
        ...
        if (config->data.general.replayGainGrouping == Config::Data::General::Album)
            item->setText(Column_File, item->url.toDisplayString(QUrl::PreferLocalFile));
        else
            item->setText(Column_File, item->url.fileName());

        if (item->tags && item->tags->tagsRead & TagData::TrackGain) {
            item->setText(Column_Track, QString().asprintf("%+.2f dB", item->tags->trackGain));
        } else {
            item->setText(Column_Track, "?");
        }
        if (item->tags && item->tags->tagsRead & TagData::AlbumGain) {
            item->setText(Column_Album, QString().asprintf("%+.2f dB", item->tags->albumGain));
        } else {
...
void ReplayGainFileList::processNextItem()
{
    ...
    for (int i = 0; i < topLevelItemCount() && count < config->data.general.numReplayGainFiles; i++) {
        ...
        if (item->type == ReplayGainFileListItem::Track) {
            if (mode == ReplayGainPlugin::Force || mode == ReplayGainPlugin::Remove || !item->tags || !(item->tags->tagsRead & TagData::TrackGain)
                || !(item->tags->tagsRead & TagData::AlbumGain))
                calcGain = true;
        } else {
...
void ReplayGainFileList::removeSelectedItems()
{
    for (int i = 0; i < topLevelItemCount(); i++) {
        ...
        if (item->type == ReplayGainFileListItem::Track && item->isSelected() && item->state != ReplayGainFileListItem::Processing) {
            ...
            for (int j = 0; j < item->childCount(); j++) {
                ReplayGainFileListItem *child = static_cast<ReplayGainFileListItem *>(item->child(j));
                if (child->type == ReplayGainFileListItem::Track && (child->isSelected() || item->isSelected())
                    && child->state != ReplayGainFileListItem::Processing) {
