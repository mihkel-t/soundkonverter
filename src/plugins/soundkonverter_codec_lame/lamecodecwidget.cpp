...
void LameCodecWidget::presetChanged(const QString &preset)
{
    ...
    if (preset == i18nc("Backend profile", "Medium")) {
        ...
        userdefinedBox->setEnabled(false);
        cPreset->setToolTip(
            i18n("This preset should generally be transparent to most people on most music\nand is already quite high in quality (~230 kbps abr)."));
    } else if (preset == i18nc("Backend profile", "Extreme")) {
        iPresetBitrate->setEnabled(false);
        cPresetFast->setEnabled(true);
        userdefinedBox->setEnabled(false);
        cPreset->setToolTip(
            i18n("If you have extremely good hearing and similar equipment, this preset will generally provide\nslightly higher quality than the standard mode "
                 "(~280 kbps abr)."));
        ...
        userdefinedBox->setEnabled(false);
        cPreset->setToolTip(
            i18n("This preset will usually be overkill for most people and most situations, but if you must have\nthe absolute highest quality with no regard "
                 "to filesize, this is the way to go (320 kbps cbr)."));
...
void LameCodecWidget::presetBitrateChanged(int bitrate)
{
    if (iPresetBitrate->isEnabled()
        && (bitrate == 80 || bitrate == 96 || bitrate == 112 || bitrate == 128 || bitrate == 160 || bitrate == 192 || bitrate == 224 || bitrate == 256
            || bitrate == 320)) {
